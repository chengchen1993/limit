...
parser.add_option('-d', '--masswindow', action = "store", type = "string", dest = "opt_masswindow", default = "W")
...
parser.add_option('--category', action = "store", type = "string", dest = "opt_wtagger_category", default = "LP")#Wtagger Category: HP, LP, ALLP
...
class DoFit:
    ...
    def __init__(self, in_channel, in_signal_sample,
        ...
        self.masswindow = options.opt_masswindow
        self.leg = TLegend()
        ...
        if options.closuretest == 0:
            ...
            if self.signal_model == "BulkGravWW" and self.masswindow == "W":
                self.mj_signal_min = 65
                self.mj_signal_max = 85
            elif self.signal_model == "BulkGravWW" and self.masswindow == "Z":
                self.mj_signal_min = 85
                self.mj_signal_max = 105
            elif self.signal_model == "WprimeWZ" and self.masswindow == "W":
                self.mj_signal_min = 65
                self.mj_signal_max = 85
            elif self.signal_model == "WprimeWZ" and self.masswindow == "Z":
                self.mj_signal_min = 85
                self.mj_signal_max = 105
            elif self.signal_model == "WprimeWZ-HVT-A" and self.masswindow == "W":
                self.mj_signal_min = 65
                self.mj_signal_max = 85
            elif self.signal_model == "WprimeWZ-HVT-A" and self.masswindow == "Z":
                self.mj_signal_min = 85
                self.mj_signal_max = 105
            else:
        ...
        self.file_Directory = "../../../../DeepJet/Wtag/CMSSW_7_1_15/src/W-tagging/data_35p867_69p2mb_rereco_puppi_full2016_afterpreapproval_0404/"


        if options.realdata == 1:
            self.file_data = (self.channel+"_PKUTree_16BH.root")#keep blind!!!!
        else:
        ...
        self.file_WJets0_mc  = (self.channel+"_PKUTree_WJets_xww.root")
        ...
        if   self.wtagger_category == "LP":
            if self.tau21_cut_min == 0.25:
                ...
                self.rrv_wtagger_eff_reweight_forV.setError(0.23)
                self.rrv_wtagger_eff_pT_extrapolation = 0.039*TMath.Log(self.signal_mass/400.)
            ...
            if self.tau21_cut_max == 1.0:
                ...
                self.rrv_wtagger_eff_reweight_forV.setError(0.11)
                self.rrv_wtagger_eff_pT_extrapolation = 0.085*TMath.Log(self.signal_mass/400.)
            else:
                print "self.tau21_cut_max = %s"%(self.tau21_cut_max)
        ...
        self.datacardsDir =  "cards_%s_closuretest%s_%s"%(options.additioninformation, options.closuretest, fit_model)
        ...
        self.plotsDir = "doFit_plots_%s_%s/closuretest%s_%s_%s_%s"%(options.additioninformation, fit_model, options.closuretest, self.channel, self.wtagger_category, self.masswindow)
        if not os.path.isdir(self.plotsDir):
            os.system("mkdir -p %s"%(self.plotsDir))


        ## workspace for limit
        self.file_rlt_root = self.datacardsDir+"/wwlvj_%s_%s_%s_%s_workspace.root"%(self.signal_sample, self.channel, self.wtagger_category, self.masswindow)
        ## datacard for the ubninned limit
        self.file_datacard_unbin = self.datacardsDir+"/wwlvj_%s_%s_%s_%s_unbin.txt"%(self.signal_sample, self.channel, self.wtagger_category, self.masswindow)
        ## workspace for the binned limit
        self.file_datacard_counting = self.datacardsDir+"/wwlvj_%s_%s_%s_%s_counting.txt"%(self.signal_sample, self.channel, self.wtagger_category, self.masswindow)
        ## appendix text file
        self.file_rlt_txt = self.datacardsDir+"/other_wwlvj_%s_%s_%s_%s.txt"%(self.signal_sample, self.channel, self.wtagger_category, self.masswindow)
        ...
        self.XS_STop_uncertainty = 0.020#  from data-driven (using btag unc)
    ...
    def make_Pdf(self, label, in_model_name, mass_spectrum= "_mj", ConstraintsList=[], ismc = 0):
        ...
        if in_model_name == "2Voig":
            ...
            rrv_sigma_voig = RooRealVar("rrv_sigma_voig"+label+"_"+self.channel, "rrv_sigma_voig"+label+"_"+self.channel, 5., 0.1, 10.)
        ...
        if in_model_name == "CB":
            ...
            rrv_mean_CB  = RooRealVar("rrv_mean_CB"+label+"_"+self.channel, "rrv_mean_CB"+label+"_"+self.channel, 84, 68, 98)
            rrv_sigma_CB = RooRealVar("rrv_sigma_CB"+label+"_"+self.channel, "rrv_sigma_CB"+label+"_"+self.channel, 7, 3, 30)
            rrv_alpha_CB = RooRealVar("rrv_alpha_CB"+label+"_"+self.channel, "rrv_alpha_CB"+label+"_"+self.channel,-2,-14,-0.05)
            rrv_n_CB     = RooRealVar("rrv_n_CB"+label+"_"+self.channel, "rrv_n_CB"+label+"_"+self.channel, 2, 0.1, 14)
        ...
        if in_model_name == "DoubleCB_mlvj":
            ...
            if label_tstring.Contains(self.signal_model+"600"):#because the M_ljv lower limit is 800GeV, so, just use right side
                ...
                rrv_sigma_CB = RooRealVar("rrv_sigma_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_sigma_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 71.6, 50 , 100)
                rrv_n1_CB     = RooRealVar("rrv_n1_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_n1_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 4.5, 0.1, 65)
                rrv_alpha1_CB = RooRealVar("rrv_alpha1_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_alpha1_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 1.36, 0.5, 3.)
                rrv_n2_CB     = RooRealVar("rrv_n2_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_n2_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 3.1, 0.1, 50)
                rrv_alpha2_CB = RooRealVar("rrv_alpha2_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_alpha2_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 2.15, 0.5, 5.)
                ...
                rrv_sigma_CB = RooRealVar("rrv_sigma_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_sigma_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 100, 50 , 200)
                rrv_n1_CB     = RooRealVar("rrv_n1_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_n1_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 4., 0.01, 35)
                rrv_alpha1_CB = RooRealVar("rrv_alpha1_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_alpha1_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 5, 0.1, 10.)
                rrv_n2_CB     = RooRealVar("rrv_n2_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_n2_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 4., 0.01, 45)
                ...
                rrv_sigma_CB = RooRealVar("rrv_sigma_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_sigma_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 155, 100 , 400.)
                rrv_n1_CB     = RooRealVar("rrv_n1_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_n1_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 7.5, 0.1, 100.)
                ...
                rrv_n2_CB     = RooRealVar("rrv_n2_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_n2_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 6., 0.1, 100.)
                rrv_alpha2_CB = RooRealVar("rrv_alpha2_CB"+label+"_"+self.channel+"_"+self.wtagger_category, "rrv_alpha2_CB"+label+"_"+self.channel+"_"+self.wtagger_category, 2., 0.5, 10.)
        ...
        if in_model_name == "ExpN":
            ...
            #rrv_n_ExpN = RooRealVar("rrv_n_ExpN"+label+"_"+self.channel, "rrv_n_ExpN"+label+"_"+self.channel, 2000, 0.01, 1e5)
        ...
        if in_model_name == "ErfExp" :
            ...
            if self.wtagger_category == "LP":
                rrv_c_ErfExp      = RooRealVar("rrv_c_ErfExp"+label+"_"+self.channel, "rrv_c_ErfExp"+label+"_"+self.channel,-0.036,-0.8, -0.0005)
                rrv_offset_ErfExp = RooRealVar("rrv_offset_ErfExp"+label+"_"+self.channel, "rrv_offset_ErfExp"+label+"_"+self.channel, 74., 20., 290)
                rrv_width_ErfExp  = RooRealVar("rrv_width_ErfExp"+label+"_"+self.channel, "rrv_width_ErfExp"+label+"_"+self.channel, 30., 5., 250.)
            else:
                rrv_c_ErfExp      = RooRealVar("rrv_c_ErfExp"+label+"_"+self.channel, "rrv_c_ErfExp"+label+"_"+self.channel,-0.036,-0.8, -0.0005)
                rrv_offset_ErfExp = RooRealVar("rrv_offset_ErfExp"+label+"_"+self.channel, "rrv_offset_ErfExp"+label+"_"+self.channel, 74., 20., 290)
                rrv_width_ErfExp  = RooRealVar("rrv_width_ErfExp"+label+"_"+self.channel, "rrv_width_ErfExp"+label+"_"+self.channel, 30., 5., 250.)
        ...
        if in_model_name == "ExpGaus":
            ...
            rrv_mean1_gaus  = RooRealVar("rrv_mean1_gaus"+label+"_"+self.channel, "rrv_mean1_gaus"+label+"_"+self.channel, 80, 70, 88)
            rrv_sigma1_gaus = RooRealVar("rrv_smgma1_gaus"+label+"_"+self.channel, "rrv_sigma1_gaus"+label+"_"+self.channel, 7., 4., 15.)
        ...
        if in_model_name == "ErfExp2Gaus":
            ...
            rrv_c_ErfExp      = RooRealVar("rrv_c_ErfExp"+label+"_"+self.channel, "rrv_c_ErfExp"+label+"_"+self.channel,-0.05,-2.2, 0.)
            rrv_offset_ErfExp = RooRealVar("rrv_offset_ErfExp"+label+"_"+self.channel, "rrv_offset_ErfExp"+label+"_"+self.channel, 100., 4., 440.)
            rrv_width_ErfExp  = RooRealVar("rrv_width_ErfExp"+label+"_"+self.channel, "rrv_width_ErfExp"+label+"_"+self.channel, 30., 4, 400.)
            erfExp = ROOT.RooErfExpPdf("erfExp"+label+"_"+self.channel, "erfExp"+label+"_"+self.channel, rrv_x, rrv_c_ErfExp, rrv_offset_ErfExp, rrv_width_ErfExp)

            rrv_mean1_gaus   = RooRealVar("rrv_mean1_gaus"+label+"_"+self.channel, "rrv_mean1_gaus"+label+"_"+self.channel, 84, 72, 98)
            rrv_mean2_gaus   = RooRealVar("rrv_mean2_gaus"+label+"_"+self.channel, "rrv_mean2_gaus"+label+"_"+self.channel, 180, 140, 210)
            rrv_sigma1_gaus  = RooRealVar("rrv_sigma1_gaus"+label+"_"+self.channel, "rrv_sigma1_gaus"+label+"_"+self.channel, 7, 2, 30)
            rrv_sigma2_gaus  = RooRealVar("rrv_sigma2_gaus"+label+"_"+self.channel, "rrv_sigma2_gaus"+label+"_"+self.channel, 10, 3, 35)
        ...
        if in_model_name == "2Gaus":
            ...
            rrv_mean1_gaus  = RooRealVar("rrv_mean1_gaus"+label+"_"+self.channel, "rrv_mean1_gaus"+label+"_"+self.channel, mean1_tmp, 60, 100)
            rrv_sigma1_gaus = RooRealVar("rrv_sigma1_gaus"+label+"_"+self.channel, "rrv_sigma1_gaus"+label+"_"+self.channel, sigma1_tmp, 3, 30)
            gaus1 = RooGaussian("gaus1"+label+"_"+self.channel, "gaus1"+label+"_"+self.channel, rrv_x, rrv_mean1_gaus, rrv_sigma1_gaus)
            ...
            rrv_scalesigma_gaus = RooRealVar("rrv_scalesigma_gaus"+label+"_"+self.channel, "rrv_scalesigma_gaus"+label+"_"+self.channel, scalesigma_tmp, 1.8, 7.4)
            rrv_sigma2_gaus     = RooFormulaVar("rrv_sigma2_gaus"+label+"_"+self.channel, "@0*@1", RooArgList(rrv_sigma1_gaus, rrv_scalesigma_gaus))
            gaus2 = RooGaussian("gaus2"+label+"_"+self.channel, "gaus2"+label+"_"+self.channel, rrv_x, rrv_mean2_gaus, rrv_sigma2_gaus)

            rrv_frac  = RooRealVar("rrv_frac"+label+"_"+self.channel, "rrv_frac"+label+"_"+self.channel, frac_tmp, 0.4, 0.85)
        ...
        ## Exp + 2Gaus for mj spectrum, for TTbar puppi LP
        if in_model_name == "2Gaus_Exp":

            print "########### 2Gaus + Exp for mj fit  ############"
            mean1_tmp      = 80.
            mean1_tmp_err      = 1.63e-01
            deltamean_tmp  = 6.
            deltamean_tmp_err  = 1.24e+00
            ...
            scalesigma_tmp_err = 2.11e-01
            frac_tmp       = 5.8765e-01
            frac_tmp_err       = 1.93660e-02

            ##rrv_mean1_gaus  = RooRealVar("rrv_mean1_gaus"+label+"_"+self.channel, "rrv_mean1_gaus"+label+"_"+self.channel, mean1_tmp, mean1_tmp-4, mean1_tmp+4)
            rrv_mean1_gaus  = RooRealVar("rrv_mean1_gaus"+label+"_"+self.channel, "rrv_mean1_gaus"+label+"_"+self.channel, mean1_tmp, mean1_tmp-10., mean1_tmp+10.)
            rrv_sigma1_gaus = RooRealVar("rrv_sigma1_gaus"+label+"_"+self.channel, "rrv_sigma1_gaus"+label+"_"+self.channel, sigma1_tmp, sigma1_tmp-6, sigma1_tmp+30)
            gaus1 = RooGaussian("gaus1"+label+"_"+self.channel, "gaus1"+label+"_"+self.channel, rrv_x, rrv_mean1_gaus, rrv_sigma1_gaus)

            rrv_mean2_gaus      = RooRealVar("rrv_mean2_gaus"+label+"_"+self.channel, "rrv_mean2_gaus"+label+"_"+self.channel, 170., 40., 400.)
            rrv_sigma2_gaus     = RooRealVar("rrv_sigma2_gaus"+label+"_"+self.channel, "rrv_sigma2_gaus"+label+"_"+self.channel, 15.,3.,70.)
            gaus2 = RooGaussian("gaus2"+label+"_"+self.channel, "gaus2"+label+"_"+self.channel, rrv_x, rrv_mean2_gaus, rrv_sigma2_gaus)

            rrv_frac_2gaus = RooRealVar("rrv_frac_2gaus"+label+"_"+self.channel, "rrv_frac_2gaus"+label+"_"+self.channel, frac_tmp, frac_tmp-frac_tmp_err*6, frac_tmp+frac_tmp_err*6)

            rrv_c_Exp       = RooRealVar("rrv_c_Exp"+label+"_"+self.channel, "rrv_c_Exp"+label+"_"+self.channel, -0.05,-1.2, 1.2)
            exp             = ROOT.RooExponential("exp"+label+"_"+self.channel, "exp"+label+"_"+self.channel, rrv_x, rrv_c_Exp)

            rrv_frac = RooRealVar("rrv_frac"+label+"_"+self.channel, "rrv_frac"+label+"_"+self.channel, 0.5, 0., 1.)
            model_pdf = RooAddPdf("model_pdf"+label+"_"+self.channel+mass_spectrum, "model_pdf"+label+"_"+self.channel+mass_spectrum, RooArgList(exp, gaus1, gaus2), RooArgList(rrv_frac, rrv_frac_2gaus), 1)
        ...
        if in_model_name == "User1":
            ...
            #else:
             #   rrv_p0     = RooRealVar("rrv_p0_User1"+label+"_"+self.channel, "rrv_p0_User1"+label+"_"+self.channel, 10.5, 0., 25.)
            #    rrv_p1 = RooRealVar("rrv_p1_User1"+label+"_"+self.channel, "rrv_p1_User1"+label+"_"+self.channel, -0.08, -0.5, 0.)
           # model_pdf = RooUser1Pdf("model_pdf"+label+"_"+self.channel+mass_spectrum, "model_pdf"+label+"_"+self.channel+mass_spectrum, rrv_x, rrv_p0, rrv_p1)
            ...
            if self.wtagger_category == "HP":
                rrv_p0     = RooRealVar("rrv_p0_User1"+label+"_"+self.channel, "rrv_p0_User1"+label+"_"+self.channel, 30, 0, 960)
                rrv_p1 = RooRealVar("rrv_p1_User1"+label+"_"+self.channel, "rrv_p1_User1"+label+"_"+self.channel, -2, -239, 0)
            else:
                rrv_p0     = RooRealVar("rrv_p0_User1"+label+"_"+self.channel, "rrv_p0_User1"+label+"_"+self.channel, 10.5, 0., 825.)
                rrv_p1 = RooRealVar("rrv_p1_User1"+label+"_"+self.channel, "rrv_p1_User1"+label+"_"+self.channel, -0.08, -211.5, 0.)
            model_pdf = RooUser1Pdf("model_pdf"+label+"_"+self.channel+mass_spectrum, "model_pdf"+label+"_"+self.channel+mass_spectrum, rrv_x, rrv_p0, rrv_p1)
        ...
        if in_model_name == "Pow" or in_model_name == "Pow_sr" :
            ...
            rrv_c = RooRealVar("rrv_c_Pow"+label+"_"+self.channel, "rrv_c_Pow"+label+"_"+self.channel, -4.6, -20, 0)
    ...
    def ControlPlots(self):
        ...
        cut= "(CategoryID == 1) && m_lvj> 600 && m_lvj<14000 && (massVhadJEC>65 && massVhadJEC<85) && l_pt> %s && MET_et>%s"%(lpt_cut, MET_cut)
    ...
    def print_limit_datacard(self, mode, params_list = []):
        ...
        if self.wtagger_category == "HP":
            ...
            datacard_out.write("\nCMS_eff_vtag_tau21_sf lnN %0.3f/%0.3f - - - %0.3f/%0.3f"%(1-self.rrv_wtagger_eff_reweight_forV.getError(), 1+self.rrv_wtagger_eff_reweight_forV.getError(), 1-self.rrv_wtagger_eff_reweight_forV.getError(), 1+self.rrv_wtagger_eff_reweight_forV.getError()))
        ### V-Tagger pT extrapolation uncertainties
        datacard_out.write("\nCMS_eff_vtag_pT  lnN %0.3f - - - %0.3f"%(1+self.rrv_wtagger_eff_pT_extrapolation, 1+self.rrv_wtagger_eff_pT_extrapolation))
        ...
        datacard_out.write("\nCMS_xww_btagger lnN %0.3f %0.3f/%0.3f %0.3f/%0.3f %0.3f/%0.3f %0.3f"%
                (1+self.btag_scale_signal_uncertainty, 1+self.btag_scale_wjets_uncertainty, 1-self.btag_scale_wjets_uncertainty, 1+self.btag_scale_ttbar_uncertainty, 1-self.btag_scale_ttbar_uncertainty, 1+self.btag_scale_singletop_uncertainty, 1-self.btag_scale_singletop_uncertainty, 1+self.btag_scale_signal_uncertainty))
        ...
        ### btag on the signal
#        datacard_out.write("\nCMS_xww_btag_eff lnN %0.3f - - - %0.3f"%(1+self.btag_scale_signal_uncertainty, 1+self.btag_scale_signal_uncertainty))#included in CMS_xww_btag
...
##        self.draw_canvas_with_pull2(rrv_x, datahist, mplot, mplotP, mplot_pull, ndof, parameters_list, "%s/m_lvj_fitting/"%(self.plotsDir), "check_postfit_workspace_for_limit", self.MODEL_4_mlvj, 0, 1)
    ...
    def read_postfit_workspaces(self, logy = 0):
        ...
        ### Taket the workspace for limits
        self.file_rlt_root_postfit = self.datacardsDir+"higgsCombinewwlvj_BulkGraviton_newxsec750_em_HP_unbin.AsymptoticLimits.mH750.root"
        file_postfit = TFile(self.file_rlt_root_postfit)
        workspace_postfit = file_postfit.Get("w")
        file_postfit.Print()
        workspace_postfit.Print()
    ...
    def get_pull(self, rrv_x, mplot_orig):
        ...
        for ipoint in range(0, hpull.GetN()):
            ...
            if TMath.Abs(y)>4 and TMath.Abs(y)<100:
                pull_edge=4###huang huang changed
...
#        gt.SetMarkerSize(0.25)
    ...
    def get_canvas(self, cname, isalpha = False):
        #tdrstyle.setTDRStyle()
        CMS_lumi.lumi_13TeV = "%2.1f fb^{-1}"%(self.GetLumi())
    ...
    def banner4Plot(self, iswithpull = 0):
        ...
        if iswithpull:
            if self.channel == "el":
                banner = TLatex(0.3, 0.96, ("CMS Preliminary, %2.1f fb^{-1} at #sqrt{s} = 13TeV, W#rightarrow e #nu "%(self.GetLumi())))
            elif self.channel == "mu":
                banner = TLatex(0.3, 0.96, ("CMS Preliminary, %2.1f fb^{-1} at #sqrt{s} = 13TeV, W#rightarrow #mu #nu "%(self.GetLumi())))
            elif self.channel == "em":
                banner = TLatex(0.3, 0.96, ("CMS Preliminary, %2.1f fb^{-1} at #sqrt{s} = 13TeV, W#rightarrow #mu, e #nu "%(self.GetLumi())))
            banner.SetNDC()
            banner.SetTextSize(0.041)
        else:
            if self.channel == "el":
                banner = TLatex(0.22, 0.96, ("CMS Preliminary, %2.1f fb^{-1} at #sqrt{s} = 13TeV, W#rightarrow e #nu "%(self.GetLumi())))
            if self.channel == "mu":
                banner = TLatex(0.22, 0.96, ("CMS Preliminary, %2.1f fb^{-1} at #sqrt{s} = 13TeV, W#rightarrow #mu #nu "%(self.GetLumi())))
            if self.channel == "em":
                banner = TLatex(0.22, 0.96, ("CMS Preliminary, %2.1f fb^{-1} at #sqrt{s} = 13TeV, W#rightarrow #mu, e #nu "%(self.GetLumi())))
            banner.SetNDC()
    ...
    def legend4Plot(self, plot, left=1, isFill=1, x_offset_low=0., y_offset_low=0., x_offset_high = 0., y_offset_high = 0., TwoCoulum = 1., isalpha=False, ismj=False):
        ...
        for obj in range(int(plot.numItems())):
            ...
            if objName == "errorband" :
                objName = "Uncertainty"
            if not (((plot.getInvisible(objName)) and (not TString(objName).Contains("Uncertainty"))) or (obj == 1) or TString(objName).Contains("invisi") or TString(objName).Contains("TLine") or objName == objName_before):
    ...
    def get_data(self):
        print "############### get_data ########################"
        self.get_mj_and_mlvj_dataset(self.file_data, "_data_xww", "jet_mass_puppi_corr")
    ...
    def fit_Signal(self, model_narrow = "DoubleCB_mlvj", model_width = "BWDoubleCB"):
        print "############# fit_Signal #################"
        ### Build the dataset
        self.get_mj_and_mlvj_dataset(self.file_signal, "_%s_xww"%(self.signal_sample), "jet_mass_puppi_corr")# to get the shape of m_lvj
    ...
    def fit_WJets(self):
        print "######################### fit_WJets ########################"
        ### Build the dataset
        self.get_mj_and_mlvj_dataset(self.file_WJets0_mc, "_WJets0_xww", "jet_mass_puppi_corr")# to get the shape of m_lvj
        self.get_mj_and_mlvj_dataset(self.file_WJets0_mc, "_WJets01_xww", "jet_mass_puppi_corr")# to get the shape of m_lvj
    ...
    def fit_TTbar(self):
        print "################################ fit_TTbar #########################################"
        ### Build the dataset
        self.get_mj_and_mlvj_dataset(self.file_TTbar_mc, "_TTbar_xww", "jet_mass_puppi_corr")# to get the shape of m_lvj
    ...
    def fit_STop(self):
        print "############################## fit_STop  #################################"
        self.get_mj_and_mlvj_dataset(self.file_STop_mc, "_STop_xww", "jet_mass_puppi_corr")
        if self.wtagger_category == "LP":
...
def pre_limit_sb_correction_without_systermatic(channel, signal_sample,
        in_mlvj_signal_region_min = 500, in_mlvj_signal_region_max = 700,
        in_mj_min=40, in_mj_max=140, in_mlvj_min=700, in_mlvj_max=5000,
